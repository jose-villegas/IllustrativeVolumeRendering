...
void UIBuilder::addBar(std::string sBarName, int iPosition[2], int iSize[2])
{
    ...
    if (iPosition != NULL) {
        TwSetParam(bar, NULL, "position", TW_PARAM_INT32, 2, iPosition);
    }

    if (iSize != NULL) {
        TwSetParam(bar, NULL, "size", TW_PARAM_INT32, 2, iSize);
    }
...
void UIBuilder::addTextList(std::string sBarName, std::string sName, std::string commaSeparatedItems, void *var, std::string sVarParams)
{
    TwType enumType = TwDefineEnumFromString(sName.c_str(), commaSeparatedItems.c_str());
    addVariable(sBarName, sName, enumType, var, sVarParams);
}
